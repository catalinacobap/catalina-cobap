...
function render() {
 ...
 ${isServicePage ? servicePage(t, short, page) : `<section class="editorial-statement section forest"><h2 class="reveal">${short.statement}</h2><a class="text-link" href="#${id('pricing')}">${short.viewServices}</a></section>
   <section class="work section bone" id="${id('work')}">
     ...
     <div class="work-grid" aria-label="${t.work}">${projects.map((project, index) => `<article class="project"><a class="project-image" href="${project.url}" ${external} aria-label="${project.format === 'pdf' ? t.viewPdf : t.visit}: ${project.name}"><img src="/images/work-${project.image}.${project.imageExtension || 'jpg'}" alt="${t.screenshot} ${project.name}" loading="lazy" width="${project.imageWidth || 1440}" height="${project.imageHeight || 1050}" /><span class="project-open"><span>${project.format === 'pdf' ? t.viewPdf : t.visit}</span><span class="project-open-arrow" aria-hidden="true">↗</span></span></a><div class="project-meta"><span class="project-number">${number(index + 1)}</span><div><p class="label">${t.projectCopy[index][0]}</p><h3><a href="${project.url}" ${external}>${project.name}</a></h3><a class="project-domain label" href="${project.url}" ${external}>${project.format === 'pdf' ? t.viewPdf : new URL(project.url).hostname}</a></div></div></article>`).join('')}</div>
   ...
   <section class="about dark" id="${id('about')}"><div class="about-image"><img src="/images/catalina.jpg" alt="${t.portrait}" loading="lazy" width="763" height="1011" /></div><div class="about-copy reveal">${kicker(7)}<h2>${language === 'es' ? 'Hola, soy' : 'Hi, I’m'}<br /><em>Catalina.</em></h2><p>${short.bio.replace('Baileyscode', `<a class="bio-link" href="${links.baileyscode}" ${external}>Baileyscode</a>`)}</p><p class="label bio-languages">${t.languages}</p>${textLink(links.linkedin, 'LinkedIn', external)}</div></section>
